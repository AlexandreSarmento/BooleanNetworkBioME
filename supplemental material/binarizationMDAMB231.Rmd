...
meanExpr <- function(timeSerie){
  
  cols <- data.EGEOD18494$codes %in% names(timeSerie)
  timeSerie <- timeSerie %>%
  ...
  aggregate(., list(symbol = timeSerie$symbol), median) %>%
...
```{r}
setwd("C:/Users/Alexandre Sarmento/Documents/R/dataAnalysis_UFRN/BooleanNetworkBioME")
...
netnodes.symbols<- c("AKT1","ATR","BAX","EGLN1","HIF1A","HIF1AN","MDM2","PFKL",
                     "PPP1R10","TP53","VEGFA","PIK3CA","INPP5D","BNIP3")

netnodes.nickname <- c("akt","atr","bax","phd","hif1a","fih","mdm2","pfkl",
                       "pnuts","tp53","vegf","pip3","ship","bnip3")
...
cellLine <- (data.EGEOD18494$cell_line == "MDA-MB231 breast cancer")

timeSerie <- 
  expr.EGEOD18494.netnodes %>%
  dplyr::select(c("symbol", data.EGEOD18494$codes[cellLine])) %>% meanExpr(.) 
...
```{r}
write.csv(t(timeSerie),"~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231/MDAMB231_.csv")
```
## GENERATING BINARIZATION HEATMAP INSIDE AN USER FOLDER
```{r}
allTimePoints = list(0,4,8,12)
cellLineName = "MDAMB231"
```
## MDAMB231 K-MEANS BIN
```{r}
setwd("~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231")
...
new.bin.basca$S1 <- new.bin.basca$S1[,-(5:6)] 
...
new.bin.bascb$S1 <- new.bin.bascb$S1[,-(5:6)] 
...
plotBinBASCB <- plotHeatMap(new.bin.bascb$S1,
                            paste(cellLineName,"_BASCB_","t_",
                            tauValue,"_s_",sigmaValue),allTimePoints)

plotBinBASCB
#knitr::include_graphics("C:/Users/Alexandre Sarmento/Documents/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231/MDAMB231_bascb_tau_0.0001_sigma_4.png")
```
## HEATMAP OF BINARIZATION THROUGH Iterative-KM 
```{r}
setwd("~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231")
filename = "~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231/IKM2/MDAMB231_"
binFileName = "~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231/IKM2/binAvg"
numberOfFiles = 100
binAvgSampleList <- readBinDataIKM(filename,binFileName,numberOfFiles)
plotBinIKM  <- plotHeatMap(binAvgSampleList$S1,"MDAMB231_iKM",allTimePoints)
plotBinIKM
#knitr::include_graphics("~/R/dataAnalysis_UFRN/BooleanNetworkBioME/mdamb231/IKM2/MDAMB231_iKM.png")
```

```{r}
# new.bin.bascb$S1
net<- reconstructNetwork(new.bin.bascb$S1,
                         method="bestfit",
                         maxK=1,
