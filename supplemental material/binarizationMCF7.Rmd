...
binBASCA <- function(v){
  ...
  binarizeMatrix(v[,-1], method = "BASCA",tau=0.01) 
...
binBASCB <- function(v,sigmaValue){
  ...
  cols <- names(v)
  binarizeMatrix(v[,-1], method = "BASCB",tau=0.01,sigma = sigmaValue)
...
load("~/folder/subfolder/data.GSE41491.RData")
```
## SETTING DATABASE 
```{r}

netnodes.symbols<- c("HIF1A","HIF1AN","EGLN1","PPP1R10","ATR","TP53","MDM2","AKT1",
                     "PTEN","PFKL","VEGFA","BAX","PIK3CA")

netnodes.nickname <- c("HIF1A","FIH","PHD","PNUTS","ATR","TP53","MDM2","AKT",
                       "PTEN","PFKL","VEGF","BAX","PIP3")
...
```{r}
cellline.breast <- data.GSE41491$cell_line == "MCF7"

breast <- 
  expr.GSE41491.netnodes %>%
  dplyr::select(c("symbol", data.GSE41491$codes[cellline.breast]))
...
```{r}
write.csv(t(breast),"~/folder/subfolder/MCF7.csv")
```
## MDAMB231 K-MEANS BIN
```{r}
breast.bin.km1 <- binKM1(breast) 
newbreast.bin.km1 = list('S1' = breast.bin.km1)
rownames(newbreast.bin.km1$S1) <- breast$symbol
...
## MDAMB231 BASCA BIN
```{r}
breast.bin.basca <- binBASCA(breast)
newbreast.bin.basca = list('S1' = breast.bin.basca)
rownames(newbreast.bin.basca$S1) <- breast$symbol
...
```{r}
sigmaValue = 4
breast.bin.bascb <- binBASCB(breast,sigmaValue)
newbreast.bin.bascb = list('S1' = breast.bin.bascb)
rownames(newbreast.bin.bascb$S1) <- breast$symbol
...
```{r}
plotHeatMap(newbreast.bin.km1$S1,"MCF7_KM1",allTimePoints)
knitr::include_graphics("~/folder/subfolder/MCF7_KM1.png")
```
## HEATMAP OF BINARIZATION THROUGH BASCA TAU = 0.01
```{r}
plotHeatMap(newbreast.bin.basca$S1,"MCF7_basca_tau_0.01",allTimePoints)
knitr::include_graphics("~/folder/subfolder/MCF7_basca_tau_0.01.png")
```
## HEATMAP OF BINARIZATION THROUGH BASCB TAU = 0.0001 sigma = 4
```{r}
plotHeatMap(newbreast.bin.bascb$S1,"MCF7_bascb_tau_0.01_sigma_4",allTimePoints)
knitr::include_graphics("~/folder/subfolder/MCF7_bascb_tau_0.01_sigma_4.png")
```
## HEATMAP OF BINARIZATION THROUGH IKM 
```{r}
filename = "~/folder/subfolder/MCF7"
binFileName = "~/folder/subfolder/binAvg"
binAvgSampleList <- readBinDataIKM(filename,binFileName)
plotHeatMap(binAvgSampleList$S1,"MCF7_iKM",allTimePoints)
knitr::include_graphics("~/folder/subfolder/MCF7_iKM.png")
```
## SAVE DATAFRAMES TO PRUNE BINARIZATIONS
```{R}
save(newbreast.bin.km1,newbreast.bin.basca,binAvgSampleList,
     newbreast.bin.bascb,file = "bin2pruneMCF7_8.RData")
```
