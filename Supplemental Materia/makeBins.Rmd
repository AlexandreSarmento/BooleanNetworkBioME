...
netnodes.symbols<- c("HIF1A","CDKN1A","ATR","TP53","MDM2","AKT1","BBC3","MIR17HG","PTEN","BCL2L11","FOXO3","CASP3","CYCS","EP300")
...
# ESTA FUNÇÃO FOI UMA GAMBIARRA QUE FIZ SOMENTE PARA QUE QUANDO PROCEDESSE COM A BINARIZAÇÃO, A VARIÁVEL DE SAÍDA APRESENTASSE 
# O NOME DO GENE A CADA LINHA
checkBin <- function(k){
  return (k*1)
}
```
...
binBASC <- function(v){
  ...
  cols <- names(v)
  binarizeMatrix(v[,-1], method = "BASCB",tau=0.01) %>%
  as.data.frame(.) %>%
...
plotHeatMap(breast.bin.basc[,2:5],"breast_BASCB_Zhou",allTimePoints)
